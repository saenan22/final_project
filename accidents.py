...
if page == "Page 1":
    ...
    st.write("""📌참고:가져온 데이터셋에서 OECD국가 중 "자동차1만대당 사망(명)" 열이 없는 국가는 모두 행 제거 처리를 했음으로 모든 OECD국가가 시각화되지 않을수 있음
    그러나 가장높은 교통사고지역을 확인할수있음.""")
...
# Page 2 내용
    ...
    with col1:
        ...
        if st.radio('교통사고 빈도가 낮은 지역 Top 5🛡️'):
...
""")
    ...
    with col2:
        if st.radio('교통사고 빈도가 높은 지역 Top 5🚨🔺'):
